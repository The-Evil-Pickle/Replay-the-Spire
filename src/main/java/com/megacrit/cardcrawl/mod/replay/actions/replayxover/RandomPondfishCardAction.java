...
public class RandomPondfishCardAction extends AbstractGameAction
{
    public boolean upgradeCard;
    
    public RandomPondfishCardAction(final boolean upgraded) {
    ...
    public void update() {
        ...
        for (final Map.Entry<String, AbstractCard> c : CardLibrary.cards.entrySet()) {
            if (c.getValue().hasTag(slimeboundbs.STUDY_PONDFISH)) {
                tmp.add(c.getKey());
            }
        }
        ...
        AbstractDungeon.actionManager.addToBottom((AbstractGameAction)new MakeTempCardInHandAction(cStudy));
        this.isDone = true;
    }
}
